...
```{r}
modelC <- tslm(formula = souvenirTrain ~ trend + season, lambda = 0)
SumC <- summary(modelC)
#modelC summary
SumC
FCC <- forecast(modelC, h = 12)
...
pander(AccC)
#An Acf
Acf(modelC$residuals, lag.max = 24)
```
...
```{r}
Acf(modelC$residuals, lag.max = 15)
ARModelSouv <- SouvenirArima <- Arima(FCC$residuals, order = c(2,0,0))
ARModelSouv
#Significance
...
ARModelSouv$coef["ar1"] / 0.1090
#AR(2) is statistically significant at 3.346186
ARModelSouv$coef["ar2"] / 0.1102
...
pander(2*pnorm(-abs((1 - SouvenirArima$coef["ar2"]) / 0.1102)), main = "ar1 using a normal distribution")
pander(2*pnorm(-abs((1 - SouvenirArima$coef["ar1"]) / 0.1090)), main = "ar1 using a normal distribution")
```
